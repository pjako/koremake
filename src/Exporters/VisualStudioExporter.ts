...
function getDirFromString(file: string, base: string): string {
	file = file.replace(/\\/g, '/');
	if (file.indexOf('/') >= 0) {
...
function getDir(file: File): string {
...
function getShaderLang() {
	...
	if (Options.graphicsApi === GraphicsApi.Direct3D9) return 'd3d9';
...
export class VisualStudioExporter extends Exporter {
	...
	userPropertyGroup(debugDir: string, indent: number) {
...
// }
	...
	exportAssetPath(project: Project, from: string, to: string, assetPath: string) {
		if (isGitPath(assetPath)) return;
		let paths = fs.readdirSync(assetPath);
		for (let p of paths) {
			if (isGitPath(p)) continue;
